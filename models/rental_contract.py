...
class rental_contract(models.Model):
    ...
    partner_id= fields.Many2one('res.partner','Tenant', required=True,domain=[('customer', '=', True)])
    ...
    account_security_deposit= fields.Many2one('account.account', 'Security Deposit Account',default=lambda self:self.env['res.config.settings'].browse( self.env['res.config.settings'].search([])[-1].id ).default_security_deposit_account.id if self.env['res.config.settings'].search([]) else "")
