...
class regions(models.Model):
  ...
  # class ProductLotes(models.Model):
  # _inherit = 'product.template'
    ...
    @api.multi
    @api.depends('terreno_id', 'terreno_id.building_area')
    def _compute_area(self):
        for rec in self:
            rec.area = sum(building.building_area for building in rec.terreno_id)

    @api.multi
    @api.depends('area', 'land_area')
    def _compute_price(self):
        for rec in self:
            rec.pricing = rec.area * rec.land_area
