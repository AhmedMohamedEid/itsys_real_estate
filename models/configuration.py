...
class real_estate_setings(models.TransientModel):
    ...
    default_security_deposit_account= fields.Many2one('account.account','Security Deposit Account',default=lambda self:self.env['res.config.settings'].browse( self.env['res.config.settings'].search([])[-1].id ).default_security_deposit_account.id if self.env['res.config.settings'].search([]) else "")
