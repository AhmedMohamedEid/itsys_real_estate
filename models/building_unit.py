...
class building_unit(models.Model):
    ...
    @api.multi
    @api.depends('land_area', 'building_area')
    def _compute_price(self):
        for rec in self:
            rec.pricing = rec.land_area * rec.building_area
    ...
    land_area= fields.Integer('Land Area m²', related="region_id.land_area", store=True, readonly=True)
    ...
    state= fields.Selection([('free','Libre'),
                               ('reserved','Reservado'),
                               ('sold','Vendido'),
                               ('blocked','Bloqueado'),
                               ], 'State',default='free' )
