...
class Parser(models.AbstractModel):
	...
	def _get_units_status(self,city_ids, region_ids, building_ids, unit_ids):
		...
		if city_ids:
			...
			regions=[]
			for region in regions_ids:regions.append(region.id)

			building_ids = self.env['building'].search([('region_id','in', regions)])
			for building in building_ids:
				units_ids = self.env['product.template'].search([('is_property', '=', True),('building_id','=', building.id)])
				for unit in units_ids:
					units.append(unit.id)
